...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _fetchUserId() async {
    ...
    setState(() {
      ...
      print('User ID: $userId'); // طباعة معرّف المستخدم
    });
    if (userId != null) {
      _fetchTickets();
    ...
    if (userId != null) {
      try {
        final response = await http.post(
          Uri.parse('http://10.0.2.2/myprojectt/get_tickets.php'),
        ...
        if (response.statusCode == 200) {
          ...
          if (data['success']) {
            setState(() {
              ...
              print('Tickets: $tickets'); // طباعة التذاكر بعد التحديث
            });
          } else {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          if (result == true) {
            _fetchTickets();
            _fetchUnreadNotificationsCount(); // إعادة تحميل التذاكر والإشعارات بعد إنشاء تذكرة جديدة بنجاح
          }
        },
        backgroundColor: Color(0xFF0288D1), // لون أزرق فاتح
